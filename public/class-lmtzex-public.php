...
class Lmtzex_Public {
	...
	public function __construct( $plugin_name, $version ) {
		...
		add_action( 'woocommerce_cart_totals_before_shipping', array( $this, 'displayCustomCheckoutField' ), 10, 1 ); # Mostrar input no carrinho
		// add_action( 'woocommerce_review_order_after_cart_contents', array( $this, 'displayCustomCheckoutField' ), 10, 1 ); # Mostrar input no checkout
		// Sempre que o escopo a box de review recarrega um novo campo é adicionado.
	...
	public function enqueue_scripts() {
		...
		if( is_checkout() || is_cart() ){
			wp_enqueue_script( 'lmtzex-datetime', plugin_dir_url( __FILE__ ) . 'js/lmtzex-public.js', array( 'jquery' ), $this->version, false );
			...
			wp_localize_script( 'lmtzex-datetime', 'delivery', array(
				'nonce'     		=> wp_create_nonce( 'charts-nonce' ),
				'hours'  			=> $this->deliveryHours(),
				'disabledWeekdays'  => $this->disabledWeekDays(),
				'disabledDatesList' => $this->disabledDates(),
				'isEnabledSameday'  => $this->deliverySameDayActived(),
			) );
		}
	}
	...
	public function checkoutDeliveryJqueryScript() {
		...
		if( is_checkout() || is_cart() && !is_wc_endpoint_url() ):
			...
			?>
			<style>
				/* Desabilitar lista de opções de entrega */
	...
	/** Método auxiliares para pegar dados de configurações */

	/**
	 * Método responsável por retornar um array de horas
	 * 
	 */
	...
	/**
	 * Método responsável por retornar datas desabilitadas
	 * 
	 */
	...
	/**
	 * Método responsável por retornar dias da semana desabilitados
	 * 
	 */
	...
	/**
	 * Método responsável por retornar bool para entrega no mesmo dia
	 * 
	 */
	public function deliverySameDayActived()
	{
		# Definição de datas desabilitadas
		$sameDayHourLimit 		= get_option( '_lmrtz_same_day_delivery_hour_limit', true );
		$sameDayMinLimit 		= get_option( '_lmrtz_same_day_delivery_min_limit', true );
